...
`import * as echarts from 'echarts';`会引入 ECharts 中所有的图表和组件，但是假如你不想引入所有组件，也可以使用 ECharts 提供的按需引入的接口来打包必须的组件。
...
我们在示例**编辑页的“完整代码”标签**提供了非常方便的**生成按需引入代码的功能**。

这个功能会根据当前的配置项动态生成最小的按需引入的代码。你可以直接在你的项目中使用。 
