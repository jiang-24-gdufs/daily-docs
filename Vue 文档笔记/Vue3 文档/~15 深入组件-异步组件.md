...
也可以在[局部注册组件](https://cn.vuejs.org/guide/components/registration.html#local-registration)时使用 `defineAsyncComponent`：
...
异步操作不可避免地会涉及到加载和错误状态，因此 `defineAsyncComponent()` 也支持在**高级选项**中处理这些状态：
...
## 搭配 Suspense 使用[#](https://cn.vuejs.org/guide/components/async.html#using-with-suspense)

异步组件可以搭配内置的 `` 组件一起使用，若想了解 `` 和异步组件之间交互，请参阅 [``](https://cn.vuejs.org/guide/built-ins/suspense.html) 章节。